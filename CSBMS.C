...
int main() {
   ...
   FILE *fp = fopen("MatchDetails.txt", "r");
   if(fp == NULL) {
       printf("Error opening file!\n");
       return 1;
   }

   // Read batsman details
   fscanf(fp, "%d", &m);   // number of batsmen
   for(i=0; i<m; i++) {
       fscanf(fp, "%s", pl1[i].name);
       fscanf(fp, "%d", &pl1[i].ones);
       fscanf(fp, "%d", &pl1[i].twos);
       fscanf(fp, "%d", &pl1[i].threes);
       fscanf(fp, "%d", &pl1[i].fours);
       fscanf(fp, "%d", &pl1[i].sixes);
       fscanf(fp, "%d", &pl1[i].balls);
   }

   // Read bowler details
   fscanf(fp, "%d", &n);   // number of bowlers
   for(i=0; i<n; i++) {
       fscanf(fp, "%s", pl2[i].name);
       fscanf(fp, "%d", &pl2[i].runsgv);
       fscanf(fp, "%d", &pl2[i].overs);
       fscanf(fp, "%d", &pl2[i].wkttkn);
   }

   fclose(fp);

   printf("All details successfully loaded from MatchDetails.txt\n");
